...
- Per-domain factories and symmetric structure keep the package self-enforcing. (There is no "architecture test" — this line used to claim one and nothing under `tests/` ever implemented it.)
...
- **Registry pattern:** Each domain with models has a registry class at its root (e.g., `Taxonomy.php`, `Commerce.php`). It resolves model classes via config so consuming projects can extend them. **Do not add `models.*` keys or registry accessors for new models.** Roughly 30 keys exist package-wide and neither consumer has overridden one in six months and two shops, while every one of them costs a static type at the callsite. An existing registry is stripped when its domain is opened for a registry-adjacent change (a model added, removed or re-homed) — not on every touch. Consumers drop the restated keys in the same bump. Re-add a key the day a consumer actually subclasses a model.
- **Contract + Concern:** Each domain ships `Contracts/Has{X}` plus a paired `Concerns/InteractsWith{X}` trait **when the trait carries default relations, scopes, or behavior** the consuming model can inherit. Pure-interface contracts that only declare a method the model must implement itself (e.g. `HasTaxCategory::taxCategory()`) ship without a trait — an empty trait is cargo-culting. **And so is a contract nothing in the package or a consumer type-hints: one capability = one contract, wherever it lives.** (`HasAvailability` and `HasStock` were the same capability minted twice, with Storefront keying availability off one and eager loading off the other.) Contracts always use the `Has*` prefix (no `*able` suffix — see Naming).
...
- **Domain events:** Dispatch a domain event when (a) a listener other than the domain's own log subscriber exists, (b) a brief names one, or (c) the event is listed in `periphery.md` as consumer-subscribable surface. Otherwise the action or model hook calls the domain logger directly. **Audit logging stays in the `{Domain}LogSubscriber` when an event exists; needing an audit row does not on its own justify minting one.** Reads and calculations never dispatch. Events are past tense (`StockAdjusted`, `PriceUpdated`), `final readonly class` with the `Dispatchable` trait, dispatched via `EventClass::dispatch(...)` and never the `event(new EventClass(...))` helper. Why the rule tightened: roughly 40 event classes existed whose only listener was the logger, and `Event::fake()` in a consumer test silently disables the audit trail that was their whole justification.
...
- **Config-driven models:** Domains that still carry a `models` key resolve classes through it. This is legacy, not the target state — see *Registry pattern* above. Morph aliases and the `$factory` statics are not part of what is being removed; they earn their keep.
...
- **Symmetry follows consumers.** "Every domain ships an Actions dir / a registry / an exception base / a report DTO" is not a reason for any of them to exist. A domain whose public surface is a contract plus a trait is allowed to be just that. Add the structure when a consumer needs it, not to make the tree look even.
- **Tripwires:** a reconcile command is the right shape for a denormalisation the package genuinely relies on under a lock — the stock ledger against `stock_items` — and **the package schedules those itself** rather than hoping a consumer does (see `scheduleWhenEnabled`). A reconcile command policing a derived counter the package could compute at read time is a smell pointing at the counter, not at the missing schedule: remove the counter. A tripwire nobody scheduled has infinite detection latency, which is the same as not having one.
- **Domain enable gates:** every `DomainServiceProvider` domain honours `{key}.enabled` (default `true`). The base `boot()` runs only the config-independent steps (config merge, morph aliases) and then calls `bootDomain()` behind the gate; migrations, translations, views, the log subscriber, commands and schedules all live in `bootDomain()`, and **a child provider puts its extras there too** — an early return in the base gates nothing a child does in its own `boot()`. Scope: `DomainServiceProvider` domains only. Currency's existing `currency.enabled` means *enabled currencies* and is untouched. ⚠ **Not implemented yet** — this is the shape PR 7 of the complexity-cleanup brief builds; new domains should be written for it now so the retrofit is smaller.
...
### Guardrails that keep the B2B seam additive

Price lists and customer groups were deleted in 2026-09 as stubs — a table plus a nullable FK encoding none of the decisions that make B2B expensive (which conditions select a price, how overlays stack, net/gross per group, VAT-ID reverse charge, registration approval). Rebuilding when a real consumer needs it costs the design work either way. What must NOT happen is the rest of the code drifting into a shape that makes the rebuild impossible, so these four hold regardless of what else changes:

- **`ResolvePrice` is the single price-resolution seam.** Consumers call `priceFor()`; they never query `prices` directly. A future group/rule overlay is then one extra context parameter on one action. Inline price queries in a consumer destroy that seam — both consumers' `app/` are clean today, and there is no test for it (a package-side test cannot reach a consumer; each consumer needs its own).
- **`prices` keeps its unique index on `(priceable, currency, minimum_quantity)`.** A future rule or group column is additive to it. Do not collapse the `minimum_quantity` tiers — they are the existing per-row price dimension a group tier would sit beside.
- **`Customer` stays a distinct model from the consumer's `User`.** Guest checkout needs it. A future group FK lands on `customers`, never on `users`.
- **Tax-inclusive vs exclusive stays ONE decision point** (`CalculateTotal` / `PriceBreakdown`), even after the B2B `TaxMode` stub is deleted. Delete the enum and the config; keep the single place a display mode would switch.
- **`QuoteCheckout` remains the only path that turns a cart into charge, lines and threshold**, so availability gates have exactly one place to hook.

The intended future shape, so the intent survives the deleted code: (1) `customer_groups` plus an FK on `customers`, assigned at registration or in the admin; (2) a price overlay resolved by *rule* (group, quantity, currency, date), not by list membership, layered on the base `prices` row; (3) per-group display mode as one switch in the total calculation; (4) per-group shipping and payment availability read from the existing config-driven zone/method tables.
