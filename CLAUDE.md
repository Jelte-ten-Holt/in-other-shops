...
- **Factories ship with the domain:** every model with a factory ships that factory in `src/{Domain}/Database/Factories/`. `newFactory()` on the model points into the package namespace. Tests — both the package's own PHPUnit suite and consumer tests — rely on the package's factories. Behaviour shared by two or more factories lives in a `{Domain}/Database/Factories/Concerns/` trait (e.g. `Commerce\...\Concerns\CreatesAddressPair`, the shipping+billing `withAddresses()` pair used by `OrderFactory` and `CustomerFactory`).
