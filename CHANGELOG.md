...
shared by both consumers. Ships with the suite's first Filament page-test
harness, which is what made them testable here.
...
- **Filament page-test harness** — `tests/Support/BootsFilament` (provider
  order is load-bearing: Filament before Livewire) and
  `tests/Stubs/Filament/StubEditableResource` with `TestEditable`; see
