...
| `Pricing\Actions\CalculateTotal` | Pricing | **Signature CHANGED (unreleased, breaking):** `__invoke(array $items, Currency $currency, int $shippingCost = 0, ?PriceList $priceList = null, ?string $voucherCode = null, ?TaxMode $taxMode = null): PriceBreakdown`. The single `int $taxRate` param is **gone**; each `items[]` entry now carries `taxRateBps`. Tax is gross-inclusive, derived per rate bracket. in-other-worlds: `App\Actions\Checkout\Steps\CalculateTotals` — update in the same release window. bianka: no checkout yet → TODO. |
