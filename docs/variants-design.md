...
| OptionValue swatch image | **`OptionValue` adopts `HasMedia`** — optional single swatch image in the `swatch` collection (added 2026-06-02, v0.29.0) | Value-level (global), for a visual storefront variant picker (Metal: silver/gold chips). Managed via a per-value `FileUpload` in `OptionResource`. No migration; no new dep edge. Storefront rendering deferred with the rest of the picker. |
...
**Phase 3 — Filament. ✅ Done.** `OptionResource` (+ pages) for the global Option/OptionValue catalog with translated name + per-value labels (manual-sync values repeater). `VariantsSchema`: `axesField` (declare axes), `variantsRepeater` + `fillFormData`/`saveFormData` (edit existing variants' SKU/price/stock, sync axes, delete removed via `DeleteVariant`). Generating new combinations (call `GenerateVariants` from a Filament Action) and the owner-price cascade modal are **deferred to Phase 5** — their value-picker UX is shaped by bianka's product form, and there's no package host to exercise them. Tests cover the manual-sync logic + component construction (no panel-render layer in this package).
