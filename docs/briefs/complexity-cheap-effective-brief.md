...
Status: **draft 2 + Release 1 reconciled** (2026-09-07). Release 1 **shipped as v0.71.1** (v0.71.0 + test-only fix). PRs: [#24 conventions](https://github.com/Jelte-ten-Holt/in-other-shops/pull/24), [#25 deletions](https://github.com/Jelte-ten-Holt/in-other-shops/pull/25), [#26 query shape](https://github.com/Jelte-ten-Holt/in-other-shops/pull/26) ([#27](https://github.com/Jelte-ten-Holt/in-other-shops/pull/27) re-opened #26 against main after the stacked merge stranded it). Source: `docs/audits/2026-09-07/complexity-audit.md` §10, top-left quadrant only. Draft 1 was attacked by a design reviewer (missed references, the seven leans, sequencing, convention wording) and a pathology reviewer (concurrency, transactions, migrations, deploy windows); every finding is folded in below. Changes from draft 1 are marked **Δ**.
...
- **Δ Every release commit gets the MySQL CI leg run on it BEFORE the tag is cut** — the leg runs only on tags, nightly and `workflow_dispatch`, so dispatch it on main by hand and wait for green (v0.71.0 was tagged before its first MySQL run and its record is red; v0.71.1 exists only to fix that). **Every DDL migration in releases 2 and 3 is run against MySQL 8 before tagging**, with the specific row shapes named in the PR (not only SQLite; FK names are schema-global, DDL is non-transactional, `up()` must be recovery-aware). Package tests still run on SQLite, so migrations branch on driver where SQLite cannot express the step (precedent: `2026_07_02_000002_restrict_shipment_items_order_line_fk.php`).
...
**Deployed 2026-09-08.** Package tagged v0.71.0 (MySQL leg red on a test-only matcher) then **v0.71.1** (green on both drivers). Both consumers bumped to `^0.71.1` and deployed the same day: Mayangna (bianka `d573eb7`) and inotherworlds.net (IOW `7327c37`), both deploy runs green incl. MySQL; `composer show` in both app containers reports v0.71.1; IOW `schedule:list` shows `inventory:reconcile` daily 03:00. Consumer suites: bianka 410, IOW 1613.

**Process lessons from the ship, for release 2:** (1) dispatch the MySQL workflow on the release commit before tagging (now a ground rule); (2) stacked PRs: merge the base first and confirm the stacked PR retargeted to main — #26 was merged into its base after #25 had already gone to main and had to be re-opened as #27; (3) a consumer bump prepared in a worktree must be re-based when the consumer's main moves (IOW PR #59 landed mid-bump) — fast-forward check first, cleanup only after a successful push.

Soak: leave release 1 on staging a few days; watch for the first `InventoryDriftDetected` (no listener yet) and confirm `domain_logs` stops gaining cart/FlowChain Info rows. Release 2 starts after the two open questions in §6 are answered.
