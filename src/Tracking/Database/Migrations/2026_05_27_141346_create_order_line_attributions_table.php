...
return new class extends Migration
{
    ...
    {
        Schema::create('order_line_attributions', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('order_line_id')->constrained()->cascadeOnDelete();
            $table->nullableMorphs('source');
            $table->timestamp('created_at')->nullable();

            // Snapshotted from cart_item_attributions at checkout. One per
            // order_line by construction (each cart_item maps to exactly
            // one order_line per the package's CreateOrder behavior).
    ...
    {
        Schema::dropIfExists('order_line_attributions');
    }
};
