...
Everything that stops the ladder is **recorded, not thrown**: non-image, missing file, non-local disk, no GD decoder for the mime, more than `media.variants.max_megapixels` (40), more pixels than the worker has memory free for (`pixels × media.variants.bytes_per_pixel` against what is left under `memory_limit` — on a 256M worker that is ~25–30 MP, since the booted worker already holds >100 MB), not wider than the smallest rung — `variants = {}` plus one `Log::info` line with the reason. So `variants IS NULL` means exactly "never attempted", which is what `media:variants` lists.
