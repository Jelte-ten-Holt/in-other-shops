...
- [ ] **A3. `ConfirmReservation` mutates ledger in place** — flips the reason of the historical `reserved` movement to `sold`, destroying the audit trail. Ledger should be append-only. Write a new compensating/linked movement instead; keep the original intact. Test required.
- [ ] **A4. Voucher usage enforcement** — `Voucher::incrementUsage()` exists but is never called. `ApplyVoucher` validates `times_used < max_uses` but never increments. Wire `incrementUsage()` into the apply flow (inside the same transaction that commits the order so races don't exceed `max_uses`). Test required.
